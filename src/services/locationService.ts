  ...
  private requestResponse(destination: string, replyTo: string, body: any, callback: (data: any, error?: string) => void) {
    ...
    const subscription = this.client.subscribe(replyTo, (message: IMessage) => {
      if (message.headers['correlation-id'] === correlationId) {
        ...
        // if the backend sends an object with an 'error' property
        if (data && data.error) {
    ...
    this.client.publish({
      destination,
      headers: { 'reply-to': replyTo, 'correlation-id': correlationId },
      body: JSON.stringify(body),
    });
  }

  fetchAll(callback: (data: Location[] | null, error?: string) => void) {
    this.requestResponse('location.get.all', 'location.get.all.res', {}, callback);
  }

  fetchById(id: number, callback: (data: Location | null, error?: string) => void) {
    this.requestResponse('location.get.id', 'location.get.id.res', id, callback);
  }

  save(location: any, callback?: (data: any, error?: string) => void) {
    this.requestResponse('location.save', 'location.save.res', location, (data, err) => {
      if (callback) {
        callback(data, err);
      }
    });
